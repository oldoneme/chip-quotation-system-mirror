...
const HierarchicalDatabaseManagement = () => {
  ...
  const handleDeleteMachineType = async (id) => {
    try {
      ...
      console.log('Machine type deleted, forcing data refresh...');
      console.log('API response:', response.data);
  ...
  const handleDeleteSupplier = async (id) => {
    try {
      ...
      message.success('删除成功');
      console.log('Supplier deleted, forcing data refresh...');
      console.log('API response:', response.data);
      forceRefreshData(); // 使用新的强制刷新机制
    } catch (error) {
  ...
  const handleDeleteMachine = async (id) => {
    try {
      ...
      message.success('删除成功');
      console.log('Machine deleted, forcing data refresh...');
      console.log('API response:', response.data);
      forceRefreshData(); // 使用新的强制刷新机制
    } catch (error) {
  ...
  const handleDeleteCardConfig = async (id) => {
    try {
      ...
      message.success('删除成功');
      console.log('Card config deleted, forcing data refresh...');
      console.log('API response:', response.data);
      forceRefreshData(); // 使用新的强制刷新机制
    } catch (error) {
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space size="middle">
          ...
          <Popconfirm
            title="确定要删除这个板卡配置吗?"
            onConfirm={() => handleDeleteCardConfig(record.id)}
            okText="确定"
            cancelText="取消"
          >
